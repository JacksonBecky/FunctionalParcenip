function separate_choose_workspace_group_gui()

  % clear all

   %[ws_FileName,ws_PathName] = uigetfile('*.mat','Select the MATLAB workspace');
   

   
   %workspace = strcat(ws_PathName,ws_FileName);
   
   workspace = dir('*.mat' );
   load(workspace.name, 'sortedmeangroupB','sorted_seeds');
    ...
    my_matrix = imagesc(sortedmeangroupB);
